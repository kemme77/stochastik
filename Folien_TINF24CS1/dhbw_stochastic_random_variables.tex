...
\begin{block}{Lean/Mathlib}
\texttt{\(\mu\).restrict s} mit der definierenden Eigenschaft:
\\[4pt]
\texttt{Measure.restrict\_apply (ht : MeasurableSet t) :}\\
\texttt{\quad (\(\mu\).restrict s) t = \(\mu\) (t \(\cap\) s)}
\end{block}
\end{frame}
...
\\[4pt]
\texttt{Measure.restrict\_mono (hs : s \(\subseteq\) t) : \(\mu\).restrict s \(\le\) \(\mu\).restrict t}
\end{block}
\end{frame}
...
\item Da \(\varphi\) einfach: \(\int_{A_n}\varphi\,d\mu\to\int_\Omega\varphi\,d\mu\) (Stetigkeit des Maßes, endliche Summe).
\end{itemize}
\item \textbf{Abschluss:} \(c\uparrow 1\) und Supremum über alle \(\varphi\le f\) liefert \(\lim\int f_n\,d\mu\ge\int f\,d\mu\).
\end{enumerate}
\end{block}
\end{frame}
