...
\title[Stochastik]
{Ein anschaulicher Einstieg in die Grundbegriffe der Stochastik \\
am Beispiel des \(n\)-fachen Münzwurfs}
...
\begin{frame}{Wie spricht man über Ereignisse?}
\begin{block}{Grundoperationen}
Für zwei Ereignisse \(A\) und \(B\) gilt:
\begin{itemize}
    \item \(A\cup B\): \text{„A oder B tritt ein“.}
    \item \(A\cap B\): \text{„A und B treten beide ein“.}
    \item \(A^{\mathsf c}=\Omega\setminus A\): \text{„A tritt nicht ein“.}
\end{itemize}
\end{block}
\end{frame}
